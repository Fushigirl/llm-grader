...
# Tesseract misreads digit images as circled numbers (e.g. "30" → "③0", "10" → "①0")
_CIRCLED_DIGIT_MAP = str.maketrans('①②③④⑤⑥⑦⑧⑨', '123456789')
...
def _normalize_circled_digits(text: str) -> str:
    """Convert OCR-garbled circled digits back to ASCII digits."""
    return text.translate(_CIRCLED_DIGIT_MAP)
...
def extract_pdf_text(pdf_path: str, ocr_last_pages: int = 0) -> str:
    ...
    def _post(text: str) -> str:
        return _normalize_circled_digits(_normalize_spaced_cjk(text))
    ...
    # 3. OCR
    try:
        return _post(_ocr_pdf(pdf_path, last_n_pages=ocr_last_pages))
    except Exception:
        return ""
