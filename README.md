...
`setup.bat` を**管理者として実行**するだけで以下が自動インストールされます。

- Python パッケージ
- Playwright ブラウザ
- Tesseract OCR + 日本語データ（未インストールの場合のみ）
...
高田研の Qwen 35B モデルを利用するために配布されているユーザー名・パスワードを使います。パスワードはコマンドライン引数には渡せず、実行時にターミナルで入力します（入力中は画面に表示されません）。
